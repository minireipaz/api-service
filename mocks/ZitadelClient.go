...
// GenerateServiceUserAccessToken provides a mock function with given fields: jwt
func (_m *ZitadelClient) GenerateServiceUserAccessToken(jwt string) (*string, time.Duration, error) {
	ret := _m.Called(jwt)

	if len(ret) == 0 {
		panic("no return value specified for GenerateServiceUserAccessToken")
	}
...
// ValidateServiceUserAccessToken provides a mock function with given fields: userToken, introspectJWT
func (_m *ZitadelClient) ValidateServiceUserAccessToken(userToken *string, introspectJWT *string) (bool, error) {
	ret := _m.Called(userToken, introspectJWT)

	if len(ret) == 0 {
		panic("no return value specified for ValidateServiceUserAccessToken")
	}
